...
<script lang="ts">
  ...
  export let useScrollout = false;
	...
	const setvhAmount = () => {
		...
		scrollytellerRef.style.setProperty('--vh', `${vh / 100}px`);
	};
...
<div
  ...
  style="
    ...
    height: {useScrollout ? `${contentHeight + (scrolloutOffset * 3)}px` : 'unset'};
  "
>
  {#if useScrollout}
  ...
  >
    <slot height={graphicHeight} width={graphicWidth} />
	</div>
  <div
